...
Hypothesis: long-short quantile ranked and z-scored dynamically risk weighted time series momentum.
'''
...
class alpha_003:
    ...
    def run(self):
        ...
        self.quantile_rank_signal = self.raw_signal.apply(lambda x: alpha_utils.quantiles(signal=x, q=0.2))

        # Convert quantile ranked signal to z-score
        self.z_score_signal = self.quantile_rank_signal.apply(lambda x: alpha_utils.z_score(x), axis=1)

        # Get indicator votes from alpha singal
        self.votes = self.z_score_signal.mask(self.z_score_signal > 0, 1).mask(self.z_score_signal < 0, -1)

        # Get risk weights from z-score
        self.risk_w = np.abs(self.z_score_signal).div(np.abs(self.z_score_signal).sum(axis=1) , axis='index')
